...
Production per-horse generative AI is deprecated and disabled. Horse visuals follow a deterministic Approved Breed Template + Genetic Phenotype Layer pipeline. Every horse permanently stores a `visual_template_id`; a stable fingerprint covers template, authoritative phenotype, pattern, face, four independent leg markings, and mane/tail treatment. Only approved and active body, base-coat, modifier, pattern, face, leg, and mane/tail assets may participate. Incomplete combinations display the official Foundation fallback rather than a contradictory approximation.
