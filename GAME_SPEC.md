...
Every Store horse receives persistent face and four-leg markings. The genetics engine resolves coat color and patterns first, then produces a structured visual phenotype containing breed/body archetype, sex, age, coat, mane/tail, pattern, and exact markings. A server-only image worker uses that structure and the approved Foundation artwork as its style reference. The resulting raster asset is stored once and follows the exact horse through purchase; page views never regenerate it. Failed jobs retain the original horse and placeholder, retry up to three times, and never reroll identity, genes, markings, or stats.
