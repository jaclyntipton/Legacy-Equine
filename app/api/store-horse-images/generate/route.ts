...
async function processJob(admin:any,reference:Buffer,job:ImageJob){const prompt=buildHorseImagePrompt(job.visual_phenotype);try{const result=await generateImage({model:gateway.imageModel("bfl/flux-2-pro"),prompt:{text:prompt,images:[reference]},aspectRatio:"4:3",maxRetries:1});const ext=result.image.mediaType.includes("jpeg")?"jpg":"png",objectPath=`generated/store/${job.horse_id}-v3.${ext}`;const uploaded=await admin.storage.from("legacy-equine-media").upload(objectPath,result.image.uint8Array,{contentType:result.image.mediaType,cacheControl:"31536000",upsert:true});if(uploaded.error)throw uploaded.error;const publicUrl=admin.storage.from("legacy-equine-media").getPublicUrl(objectPath).data.publicUrl;const horseUpdate=await admin.from("horses").update({image_url:publicUrl,image_generation_status:"complete",image_prompt_version:"store-horse-v3"}).eq("id",job.horse_id);if(horseUpdate.error)throw horseUpdate.error;await admin.from("store_horse_image_jobs").update({status:"complete",prompt,completed_at:new Date().toISOString(),last_error:null}).eq("id",job.job_id);return{ok:true as const}}catch(error){return{ok:false as const,error:await fail(admin,job,error)}}}
