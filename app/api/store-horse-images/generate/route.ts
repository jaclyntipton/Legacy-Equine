...
// Remove only light neutral pixels connected to the canvas edge. Enclosed white
// markings on the horse remain opaque, unlike a global white-to-alpha filter.
async function transparentBackground(input:Uint8Array){
 ...
 const add=(index:number)=>{if(index<0||index>=seen.length||seen[index])return;const p=index*4,r=data[p],g=data[p+1],b=data[p+2];if(Math.min(r,g,b)<190||Math.max(r,g,b)-Math.min(r,g,b)>32)return;seen[index]=1;queue[tail++]=index};
 ...
 return sharp(data,{raw:info}).flatten({background:"#f8f5ff"}).png().toBuffer();
}

async function processJob(admin:any,job:ImageJob){const prompt=buildHorseImagePrompt(job.visual_phenotype);try{const result=await generateImage({model:gateway.imageModel("bfl/flux-2-pro"),prompt,aspectRatio:"4:3",maxRetries:1}),png=await transparentBackground(result.image.uint8Array),objectPath=`generated/horses/${job.horse_id}-v11.png`;const uploaded=await admin.storage.from("legacy-equine-media").upload(objectPath,png,{contentType:"image/png",cacheControl:"31536000",upsert:true});if(uploaded.error)throw uploaded.error;const publicUrl=admin.storage.from("legacy-equine-media").getPublicUrl(objectPath).data.publicUrl,completed=await admin.rpc("complete_horse_image_job",{target_job:job.job_id,target_horse:job.horse_id,new_image_url:publicUrl,used_prompt:prompt,prompt_version:"horse-v11"});if(completed.error)throw completed.error;return{ok:true as const}}catch(error){return{ok:false as const,error:await fail(admin,job,error)}}}
