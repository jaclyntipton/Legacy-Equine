...
async function processJob(admin:any,reference:Buffer,job:ImageJob){const prompt=buildHorseImagePrompt(job.visual_phenotype);try{const result=await generateImage({model:gateway.imageModel("bfl/flux-2-pro"),prompt:{text:prompt,images:[reference]},aspectRatio:"4:3",maxRetries:1});const ext=result.image.mediaType.includes("jpeg")?"jpg":"png",objectPath=`generated/horses/${job.horse_id}-v4.${ext}`;const uploaded=await admin.storage.from("legacy-equine-media").upload(objectPath,result.image.uint8Array,{contentType:result.image.mediaType,cacheControl:"31536000",upsert:true});if(uploaded.error)throw uploaded.error;const publicUrl=admin.storage.from("legacy-equine-media").getPublicUrl(objectPath).data.publicUrl;const horseUpdate=await admin.from("horses").update({image_url:publicUrl,image_generation_status:"complete",image_prompt_version:"horse-v4"}).eq("id",job.horse_id).in("image_generation_status",["pending","processing"]).or("image_url.is.null,image_url.eq.,image_url.eq./foundation-horse.png");if(horseUpdate.error)throw horseUpdate.error;await admin.from("store_horse_image_jobs").update({status:"complete",prompt,completed_at:new Date().toISOString(),last_error:null}).eq("id",job.job_id);return{ok:true as const}}catch(error){return{ok:false as const,error:await fail(admin,job,error)}}}

export async function POST(request:Request){if(!url||!anonKey||!serviceKey)return Response.json({error:"Image worker is not configured"},{status:503});if(!await authenticate(request))return Response.json({error:"Authentication required"},{status:401});const admin=createClient(url,serviceKey,{auth:{persistSession:false}});const reference=await readFile(path.join(process.cwd(),"public","foundation-horse.png"));let completed=0,failed=0;const messages=new Set<string>();for(let batch=0;batch<10;batch++){const jobs=(await Promise.all([claim(admin),claim(admin)])).filter((job):job is ImageJob=>Boolean(job));if(!jobs.length)break;const results=await Promise.all(jobs.map(job=>processJob(admin,reference,job)));completed+=results.filter(x=>x.ok).length;failed+=results.filter(x=>!x.ok).length;for(const result of results)if(!result.ok)messages.add(result.error)}return Response.json({completed,failed,message:messages.values().next().value??null})}
