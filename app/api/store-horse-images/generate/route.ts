...
const qaSchema=z.object({approved:z.boolean(),has_signature_or_writing:z.boolean(),malformed_anatomy:z.boolean(),major_phenotype_error:z.boolean(),background_error:z.boolean(),reasons:z.array(z.string()).max(6)});
async function inspectArtwork(source:Uint8Array,job:ImageJob){const{output}=await generateText({model:gateway("openai/gpt-5.4-mini"),output:Output.object({schema:qaSchema}),messages:[{role:"user",content:[{type:"text",text:`Act as a strict horse-game artwork QA reviewer. Expected breed: ${job.visual_phenotype.breed}; sex/body type: ${job.visual_phenotype.sex}; exact coat phenotype: ${job.visual_phenotype.color}; pattern: ${job.visual_phenotype.pattern}; face marking: ${job.visual_phenotype.face_marking}; leg markings: LF ${job.visual_phenotype.left_front}, RF ${job.visual_phenotype.right_front}, LH ${job.visual_phenotype.left_hind}, RH ${job.visual_phenotype.right_hind}. Reject if there is any signature, watermark, artist mark, readable or pseudo-text, letters, numbers, logo, symbol, or writing. Reject malformed anatomy, extra/missing/fused limbs or hooves, major breed/color/pattern/marking mismatch, visible genitalia, multiple horses, props, or a physical/checkerboard background. Approve only one anatomically plausible matching horse isolated on an empty neutral/transparent background.`},{type:"file",mediaType:"image/png",data:source}]}],maxRetries:1});return output}

async function processJob(admin:any,job:ImageJob){const prompt=buildHorseImagePrompt(job.visual_phenotype);try{const result=await generateImage({model:gateway.imageModel("bfl/flux-2-pro"),prompt,aspectRatio:"4:3",maxRetries:1}),source=result.image.uint8Array,qa=await inspectArtwork(source,job);await admin.from("horse_image_qa_reviews").insert({horse_id:job.horse_id,job_id:job.job_id,approved:qa.approved,checks:qa});if(!qa.approved)throw new Error(`Artwork QA rejected: ${qa.reasons.join("; ")||"visual requirements not met"}`);const png=await transparentBackground(source),objectPath=`generated/horses/${job.horse_id}-v13.png`;const uploaded=await admin.storage.from("legacy-equine-media").upload(objectPath,png,{contentType:"image/png",cacheControl:"31536000",upsert:true});if(uploaded.error)throw uploaded.error;const publicUrl=admin.storage.from("legacy-equine-media").getPublicUrl(objectPath).data.publicUrl,completed=await admin.rpc("complete_horse_image_job",{target_job:job.job_id,target_horse:job.horse_id,new_image_url:publicUrl,used_prompt:prompt,prompt_version:"horse-v13-qa"});if(completed.error)throw completed.error;return{ok:true as const}}catch(error){return{ok:false as const,error:await fail(admin,job,error)}}}

export async function POST(request:Request){if(!url||!anonKey||!serviceKey)return Response.json({error:"Image worker is not configured"},{status:503});if(!await authenticate(request))return Response.json({error:"Authentication required"},{status:401});const admin=createClient(url,serviceKey,{auth:{persistSession:false}});let completed=0,failed=0;const messages=new Set<string>();for(let batch=0;batch<8;batch++){const job=await claim(admin);if(!job)break;const result=await processJob(admin,job);if(result.ok)completed++;else{failed++;messages.add(result.error)}}return Response.json({completed,failed,message:messages.values().next().value??null})}
