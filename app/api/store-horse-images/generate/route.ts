...
async function transparentBackground(input:Uint8Array){
 ...
 const add=(index:number)=>{if(index<0||index>=seen.length||seen[index])return;const p=index*4,r=data[p],g=data[p+1],b=data[p+2];if(Math.min(r,g,b)<220||Math.max(r,g,b)-Math.min(r,g,b)>28)return;seen[index]=1;queue[tail++]=index};
...
async function processJob(admin:any,job:ImageJob){const prompt=buildHorseImagePrompt(job.visual_phenotype);try{const result=await generateImage({model:gateway.imageModel("bfl/flux-2-pro"),prompt,aspectRatio:"4:3",maxRetries:1}),png=await transparentBackground(result.image.uint8Array),objectPath=`generated/horses/${job.horse_id}-v10.png`;const uploaded=await admin.storage.from("legacy-equine-media").upload(objectPath,png,{contentType:"image/png",cacheControl:"31536000",upsert:true});if(uploaded.error)throw uploaded.error;const publicUrl=admin.storage.from("legacy-equine-media").getPublicUrl(objectPath).data.publicUrl,completed=await admin.rpc("complete_horse_image_job",{target_job:job.job_id,target_horse:job.horse_id,new_image_url:publicUrl,used_prompt:prompt,prompt_version:"horse-v10"});if(completed.error)throw completed.error;return{ok:true as const}}catch(error){return{ok:false as const,error:await fail(admin,job,error)}}}
