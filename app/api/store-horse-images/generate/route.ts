...
async function processJob(admin:any,job:ImageJob){const prompt=buildHorseImagePrompt(job.visual_phenotype);try{const result=await generateImage({model:gateway.imageModel("bfl/flux-2-pro"),prompt,aspectRatio:"4:3",maxRetries:1});const ext=result.image.mediaType.includes("jpeg")?"jpg":"png",objectPath=`generated/horses/${job.horse_id}-v8.${ext}`;const uploaded=await admin.storage.from("legacy-equine-media").upload(objectPath,result.image.uint8Array,{contentType:result.image.mediaType,cacheControl:"31536000",upsert:true});if(uploaded.error)throw uploaded.error;const publicUrl=admin.storage.from("legacy-equine-media").getPublicUrl(objectPath).data.publicUrl;const completed=await admin.rpc("complete_horse_image_job",{target_job:job.job_id,target_horse:job.horse_id,new_image_url:publicUrl,used_prompt:prompt,prompt_version:"horse-v8"});if(completed.error)throw completed.error;return{ok:true as const}}catch(error){return{ok:false as const,error:await fail(admin,job,error)}}}
