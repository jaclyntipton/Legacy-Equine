...
export function StoreBulkPurchase({department,price,balance,disabled,maxPurchase,limitReason,onBuy}:{department:"feed"|"tack";price:number;balance:number;disabled:boolean;maxPurchase?:number|null;limitReason?:string;onBuy:(quantity:number,requestKey:string)=>Promise<boolean>}){
 const max=department==="feed"?Math.max(1,maxPurchase??1_000_000):10,quick=department==="feed"?[1,5,10,25,50]:[1,2,5,10],initial=department==="feed"?Math.min(10,max):1;
 ...
 return <div className={styles.purchase}>
  ...
  {department==="feed"&&limitReason&&<small className={styles.limit}>{limitReason}</small>}
  <button type="button" className={`primary ${styles.buy}`} disabled={disabled||buying||quantity>(maxPurchase??quantity)||balance<price*quantity} onClick={()=>void buy()}>{buying?"BUYING…":`BUY ${quantity}`}</button>
 </div>;
}
