...
describe("simulation", () => {
  it("generates deterministic foundation horses in configured bounds", () => { const a=createFoundationHorse(seededRandom(42)); const b=createFoundationHorse(seededRandom(42)); expect(a.stats).toEqual(b.stats); Object.values(a.stats).forEach(v=>expect(v).toBeGreaterThanOrEqual(GAME.foundation.outlierMinimum)); });
