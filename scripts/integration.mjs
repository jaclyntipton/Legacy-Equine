...
try{
  ...
  q=await admin.from("horses").select("id,genetics,breed_composition,color").in("id",first.data.map(h=>h.horse_id));assert.ifError(q.error);assert.equal(q.data.length,6);assert.ok(q.data.every(h=>Object.keys(h.genetics).length>=20&&Object.values(h.breed_composition).reduce((n,x)=>n+Number(x),0)===1&&h.color.length>0));
