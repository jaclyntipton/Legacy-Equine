...
async function makeUser(n){const email=`legacy-equine-test-${stamp}-${n}@example.com`;const{data,error}=await admin.auth.admin.createUser({email,password,email_confirm:true});if(error)throw error;ids.push(data.user.id);const client=createClient(url,process.env.SUPABASE_ANON_KEY,{auth:{persistSession:false}});const signed=await client.auth.signInWithPassword({email,password});if(signed.error)throw signed.error;return{client,id:data.user.id}}
try{
 const a=await makeUser(1),b=await makeUser(2);
 let r=await a.client.rpc("initialize_stable",{stable_name:"Integration Acres"});assert.ifError(r.error);const firstNumber=r.data.stable_number;
