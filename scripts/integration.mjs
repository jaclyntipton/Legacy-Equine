...
try{
  ...
  for(const [player,name] of [[a,"Integration Acres"],[b,"Sequence Farm"],[c,"Boundary Ranch"]]){
    const created=await player.client.rpc("initialize_stable",{stable_name:name});assert.ifError(created.error);assert.equal(created.data.account_number,null);assert.equal(created.data.balance,4000);
  }
  ...
  let adminCheck=await b.client.rpc("admin_list_stables");assert.ok(adminCheck.error);assert.match(adminCheck.error.message,/administrator/i);await admin.from("stables").update({is_admin:true}).eq("id",a.id);adminCheck=await a.client.rpc("admin_list_stables");assert.ifError(adminCheck.error);assert.ok(Array.isArray(adminCheck.data));let adjustment=await a.client.rpc("admin_adjust_balance",{target_stable:a.id,adjustment:123,admin_reason:"Integration admin credit"});assert.ifError(adjustment.error);assert.equal(adjustment.data.balance,4123);adjustment=await a.client.rpc("admin_adjust_balance",{target_stable:a.id,adjustment:-123,admin_reason:"Integration admin reversal"});assert.ifError(adjustment.error);assert.equal(adjustment.data.balance,4000);const customStats=Object.fromEntries(["Agility","Speed","Endurance","Temperament","Strength","Intelligence","Conformation"].map(x=>[x,99])),customMarkings={face:"blaze",left_front:"sock",right_front:"none",left_hind:"stocking",right_hind:"sock"};const custom=await a.client.rpc("admin_create_visual_horse",{target_owner:a.id,horse_name:"Admin Test Horse",horse_species:"Horse",horse_breed:"Quarter Horse",horse_sex:"Mare",horse_age_years:7,desired_color:"Palomino",desired_pattern:"Solid",desired_markings:customMarkings,horse_stats:customStats,advanced_genetics:{},horse_image_url:""});assert.ifError(custom.error);assert.equal(custom.data.stats.Speed,99);assert.equal(custom.data.is_admin_custom,true);assert.equal(custom.data.color,"Palomino");assert.deepEqual(custom.data.genetics.Cream,["Cr","N"]);assert.deepEqual(custom.data.markings,customMarkings);assert.equal(custom.data.image_generation_status,"pending");const forbiddenGrant=await a.client.rpc("owner_set_admin",{target_stable:b.id,make_admin:true});assert.ok(forbiddenGrant.error);assert.match(forbiddenGrant.error.message,/owner account/i);
  ...
  assert.ok(new Set(first.data.map(h=>h.breed)).size>1);for(const h of first.data){assert.equal(h.name,"Unnamed Foundation Horse");assert.equal(h.image_url,"/foundation-horse.png");assert.equal(Object.keys(h.stats).length,7);const gameAge=(Date.now()-new Date(h.birth_date).getTime())/86400000*30/365.25;assert.ok(gameAge>=1.99&&gameAge<2.01);}
  q=await admin.from("horses").select("id,genetics,breed_composition,color,markings,visual_phenotype,image_generation_status").in("id",first.data.map(h=>h.horse_id));assert.ifError(q.error);assert.equal(q.data.length,6);assert.ok(q.data.every(h=>Object.keys(h.genetics).length>=20&&Object.values(h.breed_composition).reduce((n,x)=>n+Number(x),0)===1&&h.color.length>0&&h.visual_phenotype.color===h.color&&Object.keys(h.markings).length===5&&["pending","processing","complete","failed"].includes(h.image_generation_status)));
  ...
  q=await admin.from("stables").select("balance,foundation_purchases").eq("id",winner.id).single();assert.deepEqual(q.data,{balance:1000,foundation_purchases:3});
  q=await admin.from("currency_ledger").select("amount,horse_id").eq("stable_id",winner.id).ilike("reason","LE Store purchase:%");assert.equal(q.data.length,3);assert.equal(q.data.reduce((sum,x)=>sum+x.amount,0),-3000);assert.ok(q.data.every(x=>bought.some(h=>h.id===x.horse_id)));
  ...
  const carrier={Frame:["O","N"],Cream:["Cr","N"],Extension:["E","N"],Agouti:["A","N"]};
  await admin.from("horses").update({sex:"Stallion",stud_fee:0,breed:"Thoroughbred",genetics:carrier,breed_composition:{Thoroughbred:1}}).eq("id",bought[0].id);await admin.from("horses").update({sex:"Mare",breed:"Arabian",genetics:carrier,breed_composition:{Arabian:1}}).eq("id",bought[1].id);
  let preview=await winner.client.rpc("get_breeding_genetic_preview",{stallion_id:bought[0].id,mare_id:bought[1].id});assert.ifError(preview.error);assert.equal(preview.data.total_lethal_percent,25);assert.equal(preview.data.result_breed,"Anglo-Arabian");assert.equal(preview.data.variant_chances.Cream,75);
  const forcedLethal={...carrier,Frame:["O","O"]};await admin.from("horses").update({genetics:forcedLethal}).in("id",[bought[0].id,bought[1].id]);let bred=await winner.client.rpc("breed_horses",{stallion_id:bought[0].id,mare_id:bought[1].id});assert.ok(bred.error);assert.match(bred.error.message,/lethal genotype/i);
  const safeSire={Frame:["N","N"],Cream:["Cr","Cr"],Extension:["E","E"],Agouti:["A","A"]},safeDam={Frame:["N","N"],Cream:["N","N"],Extension:["N","N"],Agouti:["N","N"]};await admin.from("horses").update({genetics:safeSire}).eq("id",bought[0].id);await admin.from("horses").update({genetics:safeDam}).eq("id",bought[1].id);
  bred=await winner.client.rpc("breed_horses",{stallion_id:bought[0].id,mare_id:bought[1].id});assert.ifError(bred.error);assert.equal(bred.data.sire_id,bought[0].id);assert.equal(bred.data.dam_id,bought[1].id);assert.equal(bred.data.breed,"Anglo-Arabian");assert.deepEqual(bred.data.breed_composition,{Arabian:0.5,Thoroughbred:0.5});assert.deepEqual(bred.data.genetics.Cream,["Cr","N"]);for(const value of Object.values(bred.data.stats))assert.ok(Number.isInteger(value)&&value>=1);
