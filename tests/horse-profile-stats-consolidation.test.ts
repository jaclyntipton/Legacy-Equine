...
describe("Horse Profile stats consolidation",()=>{
 ...
 it("normalizes legacy Stats deep links to Overview",()=>{expect(profile).toContain('next==="stats"||location.pathname.endsWith("/stats")');expect(profile).toContain('`/horses/${h.id}#overview`');expect(profile).toContain('addEventListener("hashchange",restore)')});
