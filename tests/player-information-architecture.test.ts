...
describe("player information architecture",()=>{
 it("separates profile identity from stable operations",()=>{expect(page).toContain('type MainView="stable"|"profile"');expect(page).toContain("Profile</button>");expect(page).toContain("Artwork Album</button>");expect(page).toContain("My Horses</button>");expect(page).toContain("Tack Room</button>");expect(page).toContain("Feed Room</button>")});
