...
describe("Owner Profession QA gameplay mode safety",()=>{
 ...
 it("changes gameplay mode only from its explicit toggle and retains the QA workspace",()=>{expect(modeToggle).toContain("setRunNormal(enabled)");expect(modeToggle).not.toContain("setCareer(");expect(modeToggle).not.toContain("history.replaceState")});
