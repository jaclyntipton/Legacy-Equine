...
describe("operational Professional Market",()=>{
 ...
 it("exposes a guided career workspace with locked sequential stages",()=>{for(const label of ['"overview"','"study"','"test"','"services"','"progression"',"Step ${step} of 4","Study Complete ✓","Career Completed ✓","← Exit Career"])expect(ui).toContain(label);expect(ui).toContain("disabled={!allowed(tab)}")});
